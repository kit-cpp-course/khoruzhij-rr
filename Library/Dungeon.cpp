...
bool Dungeon::makeRoom(int x, int y, Direction dir, bool firstRoom = false)
{
	static const int minRoomSize = 3;
	static const int maxRoomSize = 6;

	Rect room;
	room.width = MyRand::randomInt(minRoomSize, maxRoomSize);
	room.height = MyRand::randomInt(minRoomSize, maxRoomSize);
	...
	void Dungeon::generate(int maxFeatures)
	{
		...
		if (!makeRoom(_width / 2, _height / 2, static_cast<Direction>(MyRand::randomInt(4), true)))
		{
	...
	void Dungeon::print(std::ostream& out)
	{
	...
	bool Dungeon::createFeature()
	{
		for (int i = 0; i < 1000; ++i)
		{
			...
			int r = MyRand::randomInt(_exits.size());
			int x = MyRand::randomInt(_exits[r].x, _exits[r].x + _exits[r].width - 1);
			int y = MyRand::randomInt(_exits[r].y, _exits[r].y + _exits[r].height - 1);
	...
	bool Dungeon::createFeature(int x, int y, Direction dir)
	{
		...
		if (MyRand::randomInt(100) < roomChance)
		{
			if (makeRoom(x, y, dir))
			{
	...
	bool Dungeon::makeCorridor(int x, int y, Direction dir)
	{
		...
		if (MyRand::randomBool()) // horizontal corridor
		{
			corridor.width = MyRand::randomInt(minCorridorLength, maxCorridorLength);
			corridor.height = 1;
		...
		{
			corridor.width = 1;
			corridor.height = MyRand::randomInt(minCorridorLength, maxCorridorLength);
	...
	bool Dungeon::placeObject(char tile)
	{
		...
		int r = MyRand::randomInt(_rooms.size()); // choose a random room
		int x = MyRand::randomInt(_rooms[r].x + 1, _rooms[r].x + _rooms[r].width - 2);
		int y = MyRand::randomInt(_rooms[r].y + 1, _rooms[r].y + _rooms[r].height - 2);
